...
export function HomeRoute() {
  ...
  const createGuidedDemo = async () => {
    ...
    try {
      ...
      await api.commitImport(workspace.id, guidedDemoSources(locale), false);
  ...
  return <main className="garden-shell home-shell">
    <header className="masthead"><div className="journal-mark"><Sprout size={20} /><span>Research Evidence Map</span></div><div className="edition">{text('CUSTOMER RESEARCH WORKBENCH', '고객 리서치 워크벤치')}<br />{text('VERIFICATION-FIRST SYNTHESIS', '검증 우선 AI 종합')}</div><div className="masthead-tools"><LanguageToggle /><div className="masthead-status"><i className="done" />{text('LOCAL-FIRST / TRACEABLE', 'LOCAL-FIRST / 추적 가능')}</div></div></header>
    <section className="home-hero"><span className="portfolio-hero-thesis">INSPECTABLE AI SYSTEMS · RESEARCH / 01 OF 04</span><span className="kicker">{text('REDUCE THE AI VERIFICATION TAX', 'AI VERIFICATION TAX 줄이기')}</span><h1>{text('Turn AI-generated insight into decisions you can ', 'AI가 만든 인사이트를 ')}<em>{text('trace, verify, and defend.', '추적하고 검증하고 설명할 수 있는 결정으로.')}</em></h1><p>{text('Import interviews, reviews, support threads, and meeting notes. AI output stays a proposal until a human reviews the source, contradictions, and decision context.', '인터뷰, 리뷰, 고객지원 대화와 회의 노트를 가져오세요. AI 결과는 사람이 원문과 상충 근거, 의사결정 맥락을 검토하기 전까지 제안 상태로 남습니다.')}</p><div className="home-quickstart"><button type="button" className="ink-button" onClick={() => void createGuidedDemo()} disabled={demoBusy}><Play size={15} fill="currentColor" />{demoBusy ? text('Preparing guided demo…', '가이드 데모 준비 중…') : text('Try guided demo', '가이드 데모 체험')}</button><div><strong>{text('New here?', '처음이신가요?')}</strong><span>{text('Creates a synthetic workspace, runs the real analysis flow, and opens a guided walkthrough.', '가상 워크스페이스를 만들고 실제 분석 흐름을 실행한 뒤 단계별 가이드를 엽니다.')}</span></div></div></section>
    ...
    <div className="home-grid">
      ...
      <section className="workspace-index"><div className="panel-heading"><div><span>{text('RESEARCH ARCHIVE', '리서치 아카이브')}</span><h2>{text('Recent workspaces', '최근 워크스페이스')}</h2></div><Database size={20} /></div>{error ? <ErrorState detail={error} retry={() => void load()} /> : workspaces === null ? <LoadingState label={text('Reading saved research…', '저장된 리서치 불러오는 중…')} /> : workspaces.length === 0 ? <EmptyState title={text('No saved workspaces yet.', '아직 저장된 워크스페이스가 없습니다.')} detail={text('Create one to start importing source material.', '워크스페이스를 만든 뒤 원문 자료를 가져오세요.')} /> : <div className="workspace-list">{workspaces.map((workspace, index) => <button key={workspace.id} onClick={() => navigate(`/w/${workspace.id}`)}><span>0{index + 1}</span><div><b>{workspace.name}</b><p>{workspace.description || text('No research description', '리서치 설명 없음')}</p><small>{formatDate(workspace.updated_at)}</small></div><ArrowRight size={16} /></button>)}</div>}</section>
    </div>
