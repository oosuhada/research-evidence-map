...
function WorkspaceRouteInner({ mobile }: { mobile: boolean }) {
  ...
  return <main className={`garden-shell workspace-shell ${lowPower ? 'low-power' : ''}`}>
    <header className="masthead"><div className="journal-mark"><Sprout size={20} /><span>Signal Garden</span></div><div className="edition">{detail.workspace.name.toUpperCase()}<br />AI PRODUCT DISCOVERY CANVAS</div><div className="masthead-status"><i className={latestRun?.status === 'succeeded' ? 'done' : ''} />{detail.workspace.mode.toUpperCase()}{lowPower ? ' · ECO' : ''}</div></header>
    <section className="workspace-hero"><div><Link to="/" className="back-link"><ArrowLeft size={13} />Field archive</Link><span className="kicker">WORKSPACE / {detail.workspace.id.slice(0, 8)}</span><h1>{detail.workspace.name}</h1><p>{detail.workspace.description || 'No research question has been recorded for this field yet.'}</p></div><aside><div><ShieldCheck size={16} /><span>LOCAL/DEMO MODE</span><p>Sources remain in the configured local database. Retention guidance: {detail.retention_days} days. Delete sources or the workspace whenever required.</p></div><div className="history-actions"><button onClick={() => void history('undo')} disabled={!canUndo || Boolean(busy)}><Undo2 size={14} />Undo</button><button onClick={() => void history('redo')} disabled={!canRedo || Boolean(busy)}><Redo2 size={14} />Redo</button></div></aside></section>
