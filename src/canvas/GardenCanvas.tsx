...
export function GardenCanvas({ detail, selectedEvidenceId, selectedOpportunityId, focusClusterId, onEvidenceSelect, onOpportunitySelect, lowPower = false, focused = false }: Props) {
  ...
  const nodes = useMemo<Node[]>(() => {
    ...
    const evidenceNodes: Node[] = visibleEvidence.map((item) => ({
      ...
      className: 'field-node evidence-map-node',
    ...
    const opportunityNodes: Node[] = visibleOpportunities.map((item, index) => ({
      ...
      className: 'field-node opportunity-map-node', style: { width: 250 },
    }));
    return [...clusterNodes, ...evidenceNodes, ...opportunityNodes];
  }, [focused, positions.centers, positions.points, selectedEvidenceId, selectedOpportunityId, visibleClusters, visibleEvidence, visibleIds, visibleOpportunities]);
